...
```json
{
  ...
  "token_period": 259200,
  "renewable": true,
  "allowed_entity_aliases": ["nomad-cluster"]
}
```
...
- `allowed_entity_aliases` - Specifies the entity aliases that can be used by
  this token. These aliases can be set in the (Nomad server config)[https://www.nomadproject.io/docs/configuration/vault#nomad-server]
   to be used across all tasks, or on a task-by-task basis in (Nomad job config)[https://www.nomadproject.io/docs/job-specification/vault#entity_alias].
   This value is only required if using entity aliases.
